...
for(i in 1:length(focal_sites)) {  
  ...
  fit <- lm(site_target$temperature ~ site_target$air_temperature + temp_lag, data = site_target)
  ...
  for (t in 1:length(forecasted_dates)) {
    ...
    for(ens in 1:n_members){
      ...
      forecasted_temperature <- beta0 + beta1 * temp_driv$air_temperature + beta2 * last_obs_temp + rnorm(1, mean = 0, sd = model_sigma)
